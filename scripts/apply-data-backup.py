...
branch_files = {
    Path("src-tauri/src/data_backup.rs"): r'''use std::{
    collections::HashSet,
    fs,
    path::{Path, PathBuf},
...
''',
    Path("src/backup-ui.ts"): r'''import { invoke } from '@tauri-apps/api/core'
...
renderBackupPanel()
''',
    Path("src/backup-ui.css"): r'''.backup-panel {
  margin-top: 14px;
  padding-top: 14px;
...
.backup-panel__intro h4 {
  margin: 0;
  font-size: 13px;
  color: var(--text-primary, #1d1d1f);
}

.backup-panel__intro p {
  margin: 4px 0 0;
  font-size: 12px;
  line-height: 1.5;
...
if "mod data_backup;" not in lib_text:
    marker = "mod app_settings;\n"
    if marker not in lib_text:
        raise SystemExit("Could not locate app_settings module declaration")
    lib_text = lib_text.replace(marker, marker + "mod data_backup;\n", 1)
if ".plugin(data_backup::init())" not in lib_text:
    marker = "        .plugin(tauri_plugin_dialog::init())\n"
    if marker not in lib_text:
        raise SystemExit("Could not locate dialog plugin registration")
    lib_text = lib_text.replace(marker, marker + "        .plugin(data_backup::init())\n", 1)
...
if "/src/backup-ui.ts" not in settings_text:
    marker = '    <script type="module" src="/src/updater-ui.ts"></script>\n'
    if marker not in settings_text:
        raise SystemExit("Could not locate updater UI entry")
    settings_text = settings_text.replace(
        marker,
        marker + '    <script type="module" src="/src/backup-ui.ts"></script>\n',
        1,
    )
settings_html.write_text(settings_text, encoding="utf-8", newline="\n")
