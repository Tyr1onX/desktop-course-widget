...
metadata = replace_between(
    metadata,
    "fn find_teacher_fragment",
    "fn explicit_teacher_from_text",
    r'''
...
metadata = replace_between(
    metadata,
    "fn normalize_trailing_course_code",
    "fn course_name_from_text",
    r'''
...
course_marker = "fn course_name_from_text"
prefix, suffix = metadata.split(course_marker, 1)
if "character.is_ascii_punctuation()" not in suffix:
...
metadata = prefix + course_marker + suffix

metadata = re.sub(
    r"(\|\| is_common_header\(&candidate\)\n)(\s*)\{",
    r"\1        || looks_like_roster_text(&candidate)\n\2{",
    metadata,
    count=1,
)
metadata = re.sub(
    r"(\|\| looks_like_schedule_metadata\(&candidate\)\n)(\s*)\{",
    r"\1        || looks_like_roster_text(&candidate)\n\2{",
    metadata,
    count=1,
)
...
old_split = "groups.last().is_some_and(|group| group_has_card_body(group))"
if old_split not in grid:
    raise SystemExit("course card split block not found")
grid = grid.replace(
    old_split,
...
block_start = grid.index("            let parsed_anchors = course_anchors(&group);")
block_end = grid.index("            if let Some(course) = course_from_block(", block_start)
replacement = r'''
...
'''
grid = grid[:block_start] + replacement.strip() + "\n" + grid[block_end:]

section_start = grid.index("fn section_markers(")
section_end = grid.index("\nfn section_number_from_text", section_start)
section_impl = r'''
...
'''
grid = grid[:section_start] + section_impl.strip() + "\n" + grid[section_end:]
grid_path.write_text(grid, encoding="utf-8")


tests_path = Path("src-tauri/src/native_ocr/tests.rs")
tests = tests_path.read_text(encoding="utf-8")
if "normalizes_missing_course_code_brackets_and_rejects_rosters" not in tests:
    insertion = r'''
