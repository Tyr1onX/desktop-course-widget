...
def replace_function(text: str, signature: str, replacement: str) -> str:
    ...
    if not candidates:
        raise SystemExit(f"Could not locate end of {signature}")
    end = min(candidates)
    return text[:start] + replacement.rstrip() + "\n" + text[end + 1 :]


DATA_BACKUP_RS = r'''use std::{
    collections::HashSet,
    fs,
    path::{Path, PathBuf},
...
fn read_and_validate_backup(path: &Path) -> Result<BackupBundle, String> {
    ...
    let bundle: BackupBundle = serde_json::from_slice(
...
fn normalize_catalog_schedule(schedule: &mut CatalogSchedule) -> Result<(), String> {
    ...
    for (index, course) in schedule.courses.iter_mut().enumerate() {
        ...
        if course.id.is_empty() {
            return Err(format!("课表“{}”的第 {} 项课程标识为空", schedule.name, index + 1));
        }
    ...
    let normalized = validation
        .normalized_schedule
        .ok_or_else(|| format!("课表“{}”校验失败：{}", schedule.name, validation.errors.join("；")))?;
...
fn stage_bundle(stage_root: &Path, bundle: &BackupBundle) -> Result<(), String> {
    ...
    let staged_legacy = fs::read(stage_root.join(LEGACY_SCHEDULE_FILE))
        .map_err(|error| error.to_string())?;
...
Path("src-tauri/src/data_backup.rs").write_text(DATA_BACKUP_RS, encoding="utf-8", newline="\n")
...
"""
backup_types = app_settings_type + """
type BackupPreview = {
  path: string
  fileName: string
  createdAt: number
...
"""
settings_text = replace_once(settings_text, app_settings_type, backup_types, "backup frontend types")
...
data_surface = r'''function dataSurfaceMarkup(): string {
...
'''
settings_text = replace_function(settings_text, "function dataSurfaceMarkup(): string {", data_surface)

bind_data = r'''function bindDataEvents(): void {
...
'''
settings_text = replace_function(settings_text, "function bindDataEvents(): void {", bind_data)

backup_functions = r'''async function exportBackup(): Promise<void> {
  if (!desktopRuntime) {
...
'''
marker = "async function toggleAutostart(): Promise<void> {"
if marker not in settings_text:
    raise SystemExit("Could not locate autostart function")
settings_text = settings_text.replace(marker, backup_functions + marker, 1)
...
.backup-preview {
  margin-top: 16px;
  padding: 15px;
  border: 1px solid rgba(70, 109, 178, 0.2);
  border-radius: 12px;
...
.backup-preview-title strong {
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
}

.backup-preview-title span {
  padding: 3px 7px;
  border-radius: 999px;
  background: #dce9ff;
  color: #315f9e;
...
'''
if "/* Local backup and recovery */" not in css_text:
    css_text = css_text.rstrip() + backup_css + "\n"
