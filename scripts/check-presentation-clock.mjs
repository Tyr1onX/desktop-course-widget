...
assert.match(widgetPageSource, /await Promise\.all\(\[[\s\S]*sharedMotions\.map[\s\S]*animateElement\(morph[\s\S]*targetRevealParts\.map/)
...
assert.match(widgetPageSource, /targetCopies = Array\.from\(targetLayer\.querySelectorAll<HTMLElement>\('h2, \.course-location'\)\)/)
...
assert.match(widgetPageCss, /is-promoting-source strong/)
...
console.log('presentation clock, continuous semantic final-card handoff, single-shell resizing, controller, and widget wiring checks passed')
