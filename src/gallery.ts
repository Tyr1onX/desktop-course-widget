...
const cases: Array<{ title: string; description: string; options: Partial<WidgetOptions> }> = [
  { title: '正在上课', description: '主区域突出当前课程。', options: { scenario: 'current', followCount: 3 } },
  { title: '课间等待', description: '主区域自然切换到下一节。', options: { scenario: 'between', followCount: 2 } },
  ...
  { title: '浏览其他日期', description: '实时倒计时已隐藏。', options: { scenario: 'browsing', followCount: 2 } },
...
cases.forEach((item, index) => document.querySelector(`#gallery-widget-${index}`)?.append(createWidget({ ...defaultOptions, ...item.options })))
