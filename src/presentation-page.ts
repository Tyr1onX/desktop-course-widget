  ...
  <section class="controller-shell">
    ...
    <section class="controller-card" aria-labelledby="timeline-title">
      <div class="section-heading">
        ...
        <button type="button" class="quiet-button" data-preset>宣传预设 · 12 秒</button>
      </div>
      <div class="form-grid">
        ...
        <label class="field"><span>时长（秒）</span><input type="number" min="3" max="300" step="1" value="15" data-duration /></label>
...
function updateStatus(status: PresentationStatus) {
  stateText.textContent = status.active
    ? status.playing
      ? '播放中'
      : status.finished
        ? '回放完成'
  ...
  toggleButton.disabled = !status.active
  restartButton.disabled = !status.active
